...
const dsf = async (client, message) => {
	...
	try {
		...
		if (errorDetail !== 'No active event found for this world') {
			console.log(worldNumber, errorDetail ?? err?.message)
			channels.errors.send(err)
		}
	}

	const success = !comeViaWebhook ? await addCount(client, message, alt1Count) : true
	eventID = eventData ? eventData.id : eventID

	if (success) {
		await addMessageToDB(message, db, eventID)
