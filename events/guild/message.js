...
module.exports = async (client, message) => {
	 ...
	 * 			> Create a role (@event_name #1234 OR @event_name (event)) (Save role ID to DB) [{ ID: id, roleID: roleID, eventTag: 1234 }]
	 * 			> Add to calendar
	...
	try {
		...
		if (database.serverName !== 'Luke\'s Server') return;
		...
		if (message.channel.id === eventChannel) {
			...
			if (collectOneReaction.emoji.name === '❌') {
				...
				const thisCal = database.calendarID.filter(prop => (prop.year === new Date().getUTCFullYear()) && prop.month === months[monthIndex]);
				...
				const addToCal = async (date, time) => {
					...
					if (time === 'null' || date === 'null') {
						if (time === 'null' && date === 'null') {
							client.channels.cache.get(database.channels.adminChannel).send(`${last.author}, there was an error with both the  \`Time\` and \`Date\` parameters and they have been set as null. Please go update the calendar for your event.`);
						}
						else {
							// eslint-disable-next-line no-useless-escape
							client.channels.cache.get(database.channels.adminChannel).send(`${last.author}, ${time === 'null' ? 'there was an error with the  \`Time\` parameter and it has been set as null. Please go update the calendar for your event.' : 'there was an error with the  \`Date\` parameter and it has been set as null. Please go update the calendar for your event.'}`);
						}
					}
				...
				await settingsColl.updateOne({ _id: message.guild.id }, { $push: { events: { $each: [ { messageID: last.id, roleID: newRole.id, eventTag: newRole.name.slice(eventTitle[0].length + 2) } ] } } });
