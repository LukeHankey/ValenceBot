...
export const addActive = async (db) => {
	...
	const interval = setInterval(async () => {
		try {
			const metricsProfile = await fetch(
...
const clanCheck = async (users, db) => {
	try {
		let metricsProfile = await fetch(
			`https://secure.runescape.com/m=website-data/playerDetails.ws?names=%5B%22${users}%22%5D&callback=jQuery000000000000000_0000000000&_=0`
	...
	for (const names of missingNames) {
		...
		if (check) {
			...
			const xpCheck = potentialNewNames.filter((user) => {
				if (
					Number(user.totalXP) - 10_000_000 < Number(potentialChangers.totalXP) &&
					Number(user.totalXP) + 10_000_000 > Number(potentialChangers.totalXP)
				) {
					return user
				}
