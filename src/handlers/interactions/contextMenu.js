	...
	case 'Mark event as dead.':
		if ([data.merchChannel.channelID, data.merchChannel.otherChannelID].includes(interaction.channel.id)) {
			try {
				...
				if (message.reactions.cache.has('☠️')) return await interaction.editReply({ content: 'This call is already marked as dead.', ephemeral: true })
