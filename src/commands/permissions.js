		...
		case 'for': {
			...
			const displayPerms = (perms) => {
				const permList = perms.map(p => {
					...
					switch (p.type) {
					case ApplicationCommandPermissionType.Role: prefix = '<@&'; permissionType = 'Role'
						break
					case ApplicationCommandPermissionType.User: prefix = '<@!'; permissionType = 'User'
						break
					case ApplicationCommandPermissionType.Channel: prefix = '<#'; permissionType = 'Channel'
					}

					return { name: `Type: ${permissionType}`, value: `${prefix}${p.id}${suffix}\nAccess: ${p.permission ? '✅' : '❌'}`, inline: true }
				})
