	...
	run: async (client, message, args, perms, channels) => {
		...
		await settings.findOne({ _id: message.guild.id })
			.then(async res => {
				...
				const dayNum = dayCheck.indexOf(res.citadel_reset_time.day);
				...
				case 'reminders':
							...
							if (func.checkNum(args[2], 1, Infinity) && message.guild.channels.cache.has(args[2])) {
								if (args[3] === 'reset') {
									if (!args[4] || args[4].charAt(0) !== '+') {
										...
										const dayChecks = func.checkNum(dateDay.slice(0, dateDay.indexOf('d')), 0, 6) && dateDay.includes('d');
										const hourCheck = func.checkNum(dateHour.slice(0, dateHour.indexOf('h')), 0, 168) && dateHour.includes('h');
										const minCheck = func.checkNum(dateMin.slice(1, dateMin.length - 1), 0, 10080) && dateMin.includes('m');
										const totalCheck = (dateDay.slice(0, dateDay.indexOf('d')) * day) + (dateHour.slice(0, dateHour.indexOf('h')) * hour) + (dateMin.slice(1, dateMin.length - 1) * minute);
										const totalms = 604800000;

										const newDate = func.newDates(dateDay.slice(0, dateDay.indexOf('d')), dateHour.slice(0, dateHour.indexOf('h')), dateMin.slice(1, dateMin.length - 1), resetms);
					...
					default: {
						...
						res.citadel_reset_time.reminders.forEach(x => {
							if (x.dayReset === 'reset') {
								const newDate = func.newDates(`${dayCheck.indexOf(x.dayResetPlus) || +x.dayResetPlus}`, +x.hourResetPlus, +x.minResetPlus, resetms);
								const dateDays = newDate.split(' ')[0].slice(0, 3);
								const dateHours = newDate.split(' ')[4].slice(0, 2);
								const dateMins = newDate.split(' ')[4].slice(3, 5);
								if (res.citadel_reset_time.reminders.length > 0) {
							...
							if (func.checkDate(args[2], 0, 6)) {
								if (func.checkDate(args[3], 0, 23)) {
									if (func.checkDate(args[4], 0, 59)) {
										const now = Date.now();
										const newDate = func.newDates(args[2], args[3], args[4], now);
