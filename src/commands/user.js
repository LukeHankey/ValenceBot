...
export default {
	name: 'user',
	description: [
		'Lookup a clan member by Discord ID.',
		'Lookup a group of ranks.',
	...
	run: async (client, message, args, perms, db) => {
		...
		if (args.length === 1 && checkNum(args[0], 1, Infinity)) {
			...
			if (userInDiscord && findUser) {
				...
				await createEmbedForDB(
					findUser,
					{ inDisc: userInDiscord, type: 'id' },
					result,
			...
			message.channel
				.send({ embeds: [embeds[page].setFooter({ text: `Page ${page + 1} of ${embeds.length}` })] })
				.then(async (msg) => {
					await msg.react('◀️')
					await msg.react('▶️')
			...
			if (findUser) {
				...
				await createEmbedForNoDB(
					...rsName,
					'Did not find any clan member by that rsn. Make sure you typed it correctly. If they are a new clan member, they may not have been added yet.'
				)
			}
		}
	}
