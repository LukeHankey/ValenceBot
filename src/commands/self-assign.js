...
export default {
	...
	run: async (client, message, args) => {
		...
		if (args[0] === 'roles') {
			...
			if (!allAvailableRoles.length) {
				return message.channel.send({
					content: 'No roles are assignable as no roles are above my highest role.'
				})
			}
		...
		const fieldAdd = {
			name: 'Roles Added:',
			value: `\`\`\`css\n${!added.length ? 'None' : added[0].join(', ')}\`\`\``,
			inline: true
		}
		const fieldRemove = {
			name: 'Roles Removed:',
			value: `\`\`\`fix\n${!removed.length ? 'None' : removed[0].join(', ')}\`\`\``,
			inline: true
		}
